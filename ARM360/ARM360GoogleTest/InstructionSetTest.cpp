...
TEST(InstructionSet, intDivide) {
	...
	char reg3 = '3';
	int expectedValue = 5;
	set->intDivide(state, reg1, reg2, reg3);
...
TEST(InstructionSet, branchNeg) {
	...
	set->branchNeg(state, reg, expectedValue);
...
TEST(InstructionSet, branchPos) {
	...
	char reg = '1';
	state->registers[1].setValue(1);
	set->branchPos(state, reg, expectedValue);
